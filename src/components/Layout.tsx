...
const AIChatBot = () => {
  ...
  >([
    {
      role: "model",
      text: "Hi! I'm your CampusSync assistant. Ask me about your assignments or schedule.",
    },
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "model", text: "Error connecting to AI." },
      ]);
    } finally {
      setLoading(false);
    }
...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="flex h-screen w-full bg-background-light dark:bg-background-dark overflow-hidden">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-background-dark border-r border-surface-border transform transition-transform duration-300 lg:translate-x-0 ${
          ...
          <div className="flex flex-col gap-2 pt-6 border-t border-surface-border">
            ...
            <div className="flex items-center justify-between mt-4 px-2 p-2 rounded-xl bg-surface-dark border border-surface-border">
              <div className="flex items-center gap-3">
                <div
                  className="size-8 rounded-full bg-cover bg-center border border-primary"
                  style={{
